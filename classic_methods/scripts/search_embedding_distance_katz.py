"""Grid search runner for the customer-features VAE embedding-distance Katz pipeline."""
...
SEARCH_SPACE = {
    "customer_customer_weight": [0.02,0.04, 0.07, 0.1, 0.2],
    "customer_item_weight": [1.0],
...
class SearchPaths:
    ...
    embedding_distance_config_path: Path
    katz_config_path: Path
    summary_path: Path
    config_artifact_path: Path


def _build_session_code() -> str:
    # Inputs: none. Outputs: timestamp-based search session code.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run a grid search over customer-customer, customer-item, and Katz beta parameters."
    )
    parser.add_argument(
        "--search-name",
        default="embedding_distance_katz_search",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Validation repetitions per hyperparameter combination. Kept at 1 for deterministic novel-tail validation.",
    )
    parser.add_argument(
        "--users-per-repetition",
        type=int,
        default=20,
        help="Unused by deterministic novel-tail validation; retained only for compatibility.",
    )
    parser.add_argument(
        "--recommendation-k",
        type=int,
...
def _grid_rows(search_space: dict[str, list[Any]]) -> list[dict[str, Any]]:
...
def _prepare_search_roots(search_name: str, session_code: str, *, clean: bool) -> tuple[Path, Path]:
    # Inputs: search name, session code, and cleanup flag. Outputs: session-scoped runtime-config and output roots.
    search_config_root = CLASSIC_ROOT / "config" / "search_runtime" / search_name
    search_output_root = CLASSIC_ROOT / "output_search" / search_name
    if clean:
        if search_config_root.exists():
            shutil.rmtree(search_config_root, ignore_errors=True)
        if search_output_root.exists():
            shutil.rmtree(search_output_root, ignore_errors=True)
    config_root = search_config_root / session_code
    output_root = search_output_root / session_code
...
def _build_run_paths(config_root: Path, output_root: Path, run_number: int) -> SearchPaths:
    ...
    return SearchPaths(
        ...
        embedding_distance_config_path=config_dir / "from_embedding_distance.yaml",
...
def _write_runtime_configs(
    ...
    *,
    ...
    _write_text(
        paths.embedding_distance_config_path,
        f"""
        ...
        params:
          ...
          include_self: true
          symmetrize: max
          customer_customer_weight: {params['customer_customer_weight']}
          customer_item_weight: {params['customer_item_weight']}
          normalize_customer_customer: {str(bool(params['normalize_customer_customer'])).lower()}
          normalize_customer_item: {str(bool(params['normalize_customer_item'])).lower()}
    ...
    _write_text(
        ...
        stage4:
          config_path: {paths.embedding_distance_config_path}
    ...
    _write_text(
        ...
        mlflow:
          enabled: false
          tracking_uri: null
          experiment_name: embedding_distance_katz_search
...
def _flatten_result(paths: SearchPaths, params: dict[str, Any], summary: dict[str, Any]) -> dict[str, Any]:
...
def main() -> None:
    ...
    args = parse_args()
    session_code = _build_session_code()
    config_root, output_root = _prepare_search_roots(
        args.search_name,
        session_code,
    ...
    print(f"[search] session: {session_code}")
